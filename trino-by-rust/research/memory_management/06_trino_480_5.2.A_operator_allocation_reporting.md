...
sequenceDiagram
    ...
    alt roundedUp equals currentBytes
    ...
    QC->>QC: enforceUserMemoryLimit() [passes]
    QC->>MP: reserve(taskId, tag, delta)
    MP->>MP: reservedBytes += delta, freeBytes drops below 0
    ...
    Note over ILC: CAS swap in new SettableFuture then chain pool future to operator future
    ...
    MP->>MP: free() freeBytes becomes positive
    MP->>MP: future.set(null) completes pool future
    ...
    Tree->>Root: cascade
    Root->>QC: reserveMemory(tag, delta)
    QC->>QC: enforceUserMemoryLimit fails, limit exceeded
    QC-->>Op: throw ExceededMemoryLimitException
    Note over Op: Query fails immediately. No bytes reserved. Tree unchanged.
```
...
* **Two distinct failure modes from `setBytes()` — blocking vs exception:** When an operator calls `setBytes()`, exactly one of two things happens on overuse: (1) pool exhausted → bytes are reserved, a pending future is returned, operator blocks until memory is freed (recoverable — other queries freeing memory will unblock it); (2) per-query limit exceeded → `ExceededMemoryLimitException` thrown synchronously, **no bytes reserved**, tree state unchanged (fatal — query is killed). The parent-first mutation ordering in the tree ensures the exception path leaves all intermediate contexts unmodified.
* **CAS-based `updateMemoryFuture()` is lock-free cross-thread signaling:** The `AtomicReference<SettableFuture<Void>>` on `OperatorContext` bridges two threads: the driver thread (which reads it in `isWaitingForMemory()`) and the memory-freeing thread (which completes the pool future, triggering the chained listener). The CAS loop swaps in a new `SettableFuture` only when the current one is already done, then chains the pool's future to it via `addListener()`. This avoids holding any lock during the cross-thread handoff.
* **`trySetBytes()` → compact → `setBytes()` is the idiomatic allocation pattern:** `HashBuilderOperator.updateIndex()` demonstrates the standard three-step strategy: (1) try non-blocking `trySetBytes()` — if the pool can absorb the allocation, done; (2) on failure, `index.compact()` to reclaim internal fragmentation; (3) force `setBytes()` with the compacted size. This pattern lets operators self-heal before escalating to pool-level blocking.
* **Driver checks four blocking sources in priority order:** `getBlockedFuture()` checks per operator: (1) revocation in progress (`revokingOperators` map), (2) operator's own `isBlocked()` (e.g., waiting for network data), (3) `isWaitingForMemory()` (user memory), (4) `isWaitingForRevocableMemory()`. The ordering matters — revocation takes absolute priority because the operator must complete its spill before any other processing. Only when no operator is blocked and no pages moved does the driver yield via `firstFinishedFuture(blockedFutures)`.
* **`InternalLocalMemoryContext` is unsynchronized by design:** Unlike `SimpleLocalMemoryContext` (which is `synchronized`), `InternalLocalMemoryContext` has no synchronization — it relies on the driver's `exclusiveLock` guaranteeing single-threaded access. This is documented as a deliberate choice: the operator-level decorator sits inside the driver's exclusive section, so adding another lock would be pure overhead. The only cross-thread piece (`updateMemoryFuture`) uses lock-free CAS on `AtomicReference`.
